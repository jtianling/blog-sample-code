...
HGLRC ghRC;           // 渲染器环境句柄
char buffer[100];
bool gbIsUseDisplayList;
GLuint giTorus;

#define TIME_IN_FRAME (15)
#define WIDTH (600)
#define HEIGHT (600)
...
void SceneInit(int w,int h)
{
	...
	giTorus = glGenLists(2);

	glNewList(giTorus, GL_COMPILE);
		glutSolidSphere(0.1, 30, 16); // r = 1 / 20
	glEndList();

	glNewList(giTorus+1, GL_COMPILE);
	glPushMatrix();
...
int Game_Main(void *parms = NULL, int num_parms = 0)
{
	...
	int fps = CalculateFPS(dwStartTime);

	if(gbIsUseDisplayList)
	{
		sprintf(buffer, _T("OpenGL Display List Test. Display List fps=%d"), fps);
	}
	else
	{
		sprintf(buffer, _T("OpenGL Display List Test. Function fps=%d"), fps);
	}
	...
	if (KEYDOWN(VK_SPACE))
	{
		gbIsUseDisplayList = !gbIsUseDisplayList;
	}
